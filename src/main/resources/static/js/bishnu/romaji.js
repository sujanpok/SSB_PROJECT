 ...
 $(function() {
    ...
    function convertRomanToKana(original) {
        const str = original.replace(/[Ａ-Ｚａ-ｚ]/, function(s) {String.fromCharCode(s.charCodeAt(0) - 65248)}).toLowerCase(),
            len = str.length;
        let result = '',
            tmp = '',
            index = 0,
 ...
 $('#lastName_field').keyup(function(event) {
  
        let text2 = document.getElementById('lastName_field').value;
    
    const res = convertRomanToKana(text2);
    
        document.getElementById('kataLastName_field').value= res;
    });
    
     $('#firstName_field').keyup(function(event) {
  
        let text1 = document.getElementById('firstName_field').value;
    
    const res = convertRomanToKana(text1);
    
        document.getElementById('kataFirstName_field').value= res;
    });
